...
struct VideoTexture::InternalData {
    ...
    TextureForVideo* target_texture = nullptr; 
    ...
    void update(float elapsed)
    {
        ...
        // フレーム間の経過時間を取得（1フレームごとに同期）
        video_time +=(elapsed * 1000000 );
        
        if (static_cast<int>(video_time) % 2000000 != 0) {
            return;
        }
        if (video_time < llVideoTimeStamp )
            return;
        ...
        llVideoTimeStamp =0;
        //0.5倍速にするため、2で割り切れない時間ならスキップ
        if ((llVideoTimeStamp % 2) != 0) {
        ...
        if (pVideoSample) {
            ...
            if (FAILED(hr)) {
                printf("Error: ConvertToContiguousBuffer failed\n");
                return;
            }

            hr = buf->GetCurrentLength(&bufLength);
            if (FAILED(hr)) {
                printf("Error: GetCurrentLength failed\n");
                return;
            }
            ...
            if (FAILED(hr)) {
                printf("Error: Buffer Lock failed\n");
                return;
            }
            ...
            if (!target_texture) {
                target_texture = new TextureForVideo();
...
bool VideoTexture::create(const char* filename) {
    ...
    bool result = internal_data->open(filename);
    if (!result) {
        std::cerr << "VideoTexture::create - Failed to open video file: " << filename << std::endl;
    }
    else {
        std::cerr << "VideoTexture::create - Successfully opened video file: " << filename << std::endl;
    }
...
TextureForVideo* VideoTexture::getTexture() {
    assert(internal_data);
...
void VideoTexture::setLoop(bool loop)
{
    assert(internal_data);
    internal_data->autoloop = loop;
}
