...
void parseNunchuck(const u8* data, NunchuckState& state)
{
	...
	state.joystick = (state.joystickRaw - state.joystickCalibration.offset);
...
{ 
	...
	identifyMotionPlus(); //WiiMotionPlus‚ð—LŒø‚É‚·‚é
...
void Wiimote::update()
{
	...
	if ((next.extensionType == ExtensionTypes::Nunchuck || next.extensionType == ExtensionTypes::Nunchuck_TR) && (_features | WiimoteFeatures::Extension))
	{
		...
		if (_nunchuckJoystickCallback != nullptr)
		{
			if (!_filterNunchuckJoystick || (next.nunchuck.joystick.x != old.nunchuck.joystick.x || next.nunchuck.joystick.y != old.nunchuck.joystick.y))
			{
				_nunchuckJoystickCallback(next.nunchuck.joystick);
			}
		}
	}
